...
Since $Bu + c$ is a constant over the time interval $[0, T_k)$ where $T_k$ is
the sample period for timestep $k$, this equation can be integrated according to
appendix F.2 of \textit{Controls Engineering in FRC} \cite{bib:controls-in-frc},
which gives
\begin{equation*}
  x_{k+1} = e^{AT_k} x_k + A^{-1} (e^{AT_k} - 1) (Bu_k + c)
\end{equation*}

Substitute $A$, $B$, and $c$ back in and simplify.
\begin{equation*}
  \begin{aligned}
    x_{k+1} =& e^{AT_k} x_k + A^{-1} (e^{AT_k} - 1)Bu_k +
      A^{-1} (e^{AT_k} - 1)c \\
    x_{k+1} =& e^{-\frac{K_v}{K_a} T_k} x_k \\
      +& \left(-\frac{K_v}{K_a}\right)^{-1}
        \left(e^{-\frac{K_v}{K_a} T_k} - 1\right)
    ...
    x_{k+1} =& e^{-\frac{K_v}{K_a} T_k} x_k \\
      +& \frac{1}{K_v}
        \left(1 - e^{-\frac{K_v}{K_a} T_k}\right) u_k \\
      +& \frac{K_s}{K_v}
        \left(e^{-\frac{K_v}{K_a} T_k} - 1\right)\sgn(x_k)
  \end{aligned}
\end{equation*}
...
\begin{equation*}
  \begin{cases}
    \alpha = e^{-\frac{K_v}{K_a} T_k} \\
    \beta = \frac{1}{K_v}\left(1 - e^{-\frac{K_v}{K_a} T_k}\right) \\
    \gamma = \frac{K_s}{K_v}\left(e^{-\frac{K_v}{K_a} T_k} - 1\right)
  \end{cases}
\end{equation*}
...
where $T$ is the average sample period of the velocity data.
...
In data fitting, where the goal is to find the parameters $\bm{\beta}$ such
that a given model function $y = f(x, \bm{\beta})$ best fits some data points
$(x_i, y_i)$, the functions $r_i$ are the residuals:
\begin{equation*}
  r_i(\bm{\beta}) = y_i - f(x_i, \bm{\beta})
\end{equation*}

Then, the Gauss-Newton method can be expressed in terms of the Jacobian
$\mat{J_f}$ of the function $f$ as
\begin{equation*}
  \bm{\beta}^{(s+1)} = \bm{\beta}^{(s)} +
    (\mat{J_f}^T \mat{J_f})^{-1} \mat{J_f}^T \mat{r}\left(\bm{\beta}^{(s)}\right)
\end{equation*}

\subsection{Specialization}
\begin{equation*}
  \begin{aligned}
    x_{k+1} =& f(x_k, u_k, T_k, \bm{\beta}) \\
    x_{k+1} =& e^{-\frac{K_v}{K_a} T_k} x_k \\
      +& \frac{1}{K_v}
        \left(1 - e^{-\frac{K_v}{K_a} T_k}\right) u_k \\
      +& \frac{K_s}{K_v}
...
\end{equation*}
\begin{equation*}
  \bm{\beta} = \begin{bmatrix}K_s \\ K_v \\ K_a\end{bmatrix}
\end{equation*}

% derivative of e^(-v/a t) x + 1/v (1 - e^(-v/a t)) u + s/v (e^(-v/a t) - 1)
\begin{align*}
  \mat{J}_{f,k} &= \frac{\partial f(x_k, u_k, T_k, \bm{\beta})}
    {\partial \bm{\beta}} \\
  \mat{J}_{f,k} &=
  \left[
  \begin{smallmatrix}
    % with respect to s
    \frac{1}{K_v}\left(e^{-\frac{K_v}{K_a} T_k} - 1\right) \\
    % with respect to v
    \frac{e^{-\frac{K_v}{K_a} T_k} \left(K_a (K_s\sgn(x_k) - u_k)
      \left(e^{\frac{K_v}{K_a} T_k} - 1\right) -
      T_k K_v (K_s\sgn(x_k) - u_k + K_v x_k)\right)}
      {K_a K_v^2} \\
    % with respect to a
    \frac{T_k e^{-\frac{K_v}{K_a} T_k} (K_s\sgn(x_k) - u_k + K_v x_k)}{K_a^2}
  \end{smallmatrix}
  \right]^T
\end{align*}

Let $a_k = e^{-\frac{K_v}{K_a} T_k}$ and $b_k = K_s\sgn(x_k) - u_k + K_v x_k$.
\begin{align*}
  \mat{J}_{f,k} &=
  \begin{bmatrix}
    % with respect to s
    \frac{1}{K_v}\left(a_k - 1\right) \\
    % with respect to v
    \frac{a_k \left(K_a (K_s\sgn(x_k) - u_k)\left(e^{\frac{K_v}{K_a} T_k} - 1\right) -
      T_k K_v b_k\right)}{K_a K_v^2} \\
    % with respect to a
    \frac{T_k a_k b_k}{K_a^2}
  \end{bmatrix}^T
\end{align*}
\begin{equation*}
  r_k(\bm{\beta}) = x_{k+1} - f(x_k, u_k, T_k, \bm{\beta})
\end{equation*}

Then, the Gauss-Newton method can be expressed in terms of the Jacobian
$\mat{J_{f}}$ of the function $f$ as
\begin{equation*}
  \bm{\beta}^{(s+1)} = \bm{\beta}^{(s)} +
    (\mat{J_f}^T \mat{J_f})^{-1} \mat{J_f}^T \mat{r}\left(\bm{\beta}^{(s)}\right)
\end{equation*}
